...
import Encryption_Data as AES_B64
import time
import uuid
import json
import random
...
def setupInboundTransmission():
    ...
    if (var_forwardType=="db"):
        ...
        except DatabaseError as xxx_todo_changeme1:
        ...
        try:
            ...
            #print "query=",execQuery
            results = db_handle.query(execQuery)
        except DatabaseError as xxx_todo_changeme2:
...
def processData(parm_allData,var_transId):
    ...
    for var_lineData in [1]:  # this is a short term fix to over come data  not coming in an array. 
        var_lineData=parm_allData
        print("data=",var_lineData)
        if (var_forwardType=="ff"):
            var_inDir=str(glb_lastRecvConfigParms['Settings']['input_folder'])
            var_outDir=str(glb_lastRecvConfigParms['Settings']['output_folder'])
            var_inDir=var_inDir.replace("<base>",glb_jpipeBaseDir);
            var_outDir=var_outDir.replace("<base>",glb_jpipeBaseDir);
            fileName=var_inDir+"/inData_"+str(var_transId)+"."+str(int(time.time()%1000000000))
            #print fileName 
            if (not(os.path.isdir(var_inDir))):
                print("dir ",var_outDir, "not present. Setting up")
                setupInboundTransmission()
            print("writing data to ",fileName)
            file1 = open(fileName,"w+")
            file1.write(str(var_lineData))
            file1.close()
            ...
            try:
                ...
                txn.commit()
            except DatabaseError as xxx_todo_changeme:
...
def on_connect(client,user_data,flags,rc):
    print(("connected with result code"+str(rc)+" subscribing to "+str(devRegSubTopic)))
...
def on_message(client,userdata,msg):
    ...
    strToDecode1 = msg.payload
    strToDecode=str(strToDecode1.decode("utf-8"))
    print (type(strToDecode),strToDecode)
    #print strToDecode.decode('ascii')
    appendText = cipher.decrypt(str(strToDecode))
    #tempText=appendText.replace('\\\"','\"')
    #replText=tempText.replace('\'','\"')
    print ("data=",appendText)
    try:
        inpData = json.loads(appendText)
    except ValueError:
...
client.connect("c001.jp.ejts.in", 1883, 60)
