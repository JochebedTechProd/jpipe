...
while(1):
	...
	if ( tdelta >= (datetime.strptime(alert_frequency, "%H:%M:%S") - datetime.strptime("00:00:00", "%H:%M:%S"))):
		...
		t1 = int(curr_time - (value1*60)) % 1000000000
		t2 = int(curr_time - (value2*60)) % 1000000000

		#t1 = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(t1))
		#t2 = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(t2))

		#MAIN SQL QUERY : sqlq = "select data from inData WHERE inserrtDt BETWEEN \"" + t2 + "\" AND \"" +  t1 + "\""
		...
		for file in files_in_dir:
			...
			time_position = file.find('.') + 1
			
			f_time_stamp = file[time_position:]

			if not f_time_stamp.isdigit():
				continue

			f_time_stamp = int(f_time_stamp)

			print "File time stamp : " + str(f_time_stamp)
			
			if t2 > t1 and f_time_stamp < t2 and f_time_stamp > t1:

				print "reading file: " + file
			
				file_content = open(str(file), "r")

				file_content = file_content.read()

				print file_content

				file_content = list(str(file_content))

				print file_content
			
				for i in range(len(file_content)):
			
					if file_content[i] == 'u' and file_content[i+1] == "'":
			
						file_content.pop(i)
			
				file_content = ''.join(file_content)

				print "Appending file : " + file_content
